...
exports.addContent = async (req, res) => {
	const { type, title, description, semester, fileUrl, thumbnailUrl, eventDate, category } = req.body;
	try {
		// Make sure fileUrl is always set
		const finalFileUrl = fileUrl || 'https://via.placeholder.com/800x600?text=No+Image';
	...
	try {
		...
		for (let row of data) {
			try {
				...
				if (!row.studentName || !row.courseTitle || !row.semester || !row.marks) {
				...
				if (isNaN(semesterNum) || semesterNum < 1 || semesterNum > 8) {
					errors.push(`Invalid semester value for student ${row.studentName}: ${row.semester}`);
					continue;
				}
				...
				if (isNaN(marks) || marks < 0 || marks > 100) {
					errors.push(`Invalid marks value for student ${row.studentName}: ${row.marks}`);
					continue;
				}
				...
				errors.push(`Error processing row for student ${row.studentName}: ${error.message}`);
			}
		}
